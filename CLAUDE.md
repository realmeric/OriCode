...
3. **The app never touches an agent's login.** No login screen and no token, Anthropic's included. Each agent runs through its own CLI and the login its user made in Terminal: the engine runs that CLI as it is, asks it whether it's signed in, and never reads a file or a keychain item that holds its credential. If a login is missing, the app says the one Terminal line that makes it, "run `claude` in Terminal and log in", and nothing more. The one credential OriCode keeps is a model API's key (Z.ai, DeepSeek, Meta, OpenRouter, Command Code), typed into a native secure field in Settings, kept in the Keychain and handed only to the process that calls that API. A login a maker forbids for third-party apps, Antigravity with a Google account or pi signed into claude.ai, xAI or Meta, stays off until it's turned on under the maker's own sentence. The previous version settled on the first half of this as the line that keeps a personal client inside Anthropic's terms; keep it for every maker.
